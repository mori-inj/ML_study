...
def generator( z , reuse = False ):
    ...
    with tf.variable_scope(name_or_scope = 'Gen') as scope:
        ...
        output = tf.layers.conv2d_transpose(output, l[0], 3, strides=4, padding='SAME', name='dcv0') # 16*16
        ...
        output = tf.layers.conv2d_transpose(output, l[2], 3, strides=4, padding='SAME', name='dcv2') # 64*64
...
def discriminator( x , reuse = False):
    ...
    with tf.variable_scope(name_or_scope = 'Dis', reuse=reuse) as scope:
        output = tf.layers.conv2d(x, l[0], 3, strides=4, padding='SAME', name='cv0') # 128*128 8
        ...
        output = tf.layers.conv2d(output, l[2], 3, strides=4, padding='SAME', name='cv2') # 32*32 32
        ...
        output = tf.layers.conv2d(output, l[4], 3, strides=4, padding='SAME', name='cv4') # 8*8 128
