...
def generator( z , reuse = False ):
    ...
    with tf.variable_scope(name_or_scope = 'Gen') as scope:
        output = tf.layers.dense(z, random_size * 8 * 8, name='fc')
        output = tf.reshape(output, [-1, 8, 8, random_size])

        output = tf.layers.conv2d_transpose(output, l[0], 3, strides=2, padding='SAME', name='dcv0') # 16*16
        output = tf.nn.relu(tf.layers.batch_normalization(output, name='bn0'))
        output = tf.layers.conv2d_transpose(output, l[1], 3, strides=2, padding='SAME', name='dcv1') # 32*32
        output = tf.nn.relu(tf.layers.batch_normalization(output, name='bn1'))
        output = tf.layers.conv2d_transpose(output, l[2], 3, strides=2, padding='SAME', name='dcv2') # 64*64
        output = tf.nn.relu(tf.layers.batch_normalization(output, name='bn2'))
        output = tf.layers.conv2d_transpose(output, l[3], 3, strides=2, padding='SAME', name='dcv3') # 128*128
        output = tf.nn.relu(tf.layers.batch_normalization(output, name='bn3'))
        output = tf.layers.conv2d_transpose(output, l[4], 3, strides=2, padding='SAME', name='dcv4') # 256*256
...
def discriminator( x , reuse = False):
    ...
    with tf.variable_scope(name_or_scope = 'Dis', reuse=reuse) as scope:
        output = tf.layers.conv2d(x, l[0], 3, strides=2, padding='SAME', name='cv0') # 128*128 8
        output = tf.nn.leaky_relu(tf.layers.batch_normalization(output, name='bn0'))
        output = tf.layers.conv2d(output, l[1], 3, strides=2, padding='SAME', name='cv1') # 64*64 16
        output = tf.nn.leaky_relu(tf.layers.batch_normalization(output, name='bn1'))
        output = tf.layers.conv2d(output, l[2], 3, strides=2, padding='SAME', name='cv2') # 32*32 32
        output = tf.nn.leaky_relu(tf.layers.batch_normalization(output, name='bn2'))
        output = tf.layers.conv2d(output, l[3], 3, strides=2, padding='SAME', name='cv3') # 16*16 64
        output = tf.nn.leaky_relu(tf.layers.batch_normalization(output, name='bn3'))
        output = tf.layers.conv2d(output, l[4], 3, strides=2, padding='SAME', name='cv4') # 8*8 128
